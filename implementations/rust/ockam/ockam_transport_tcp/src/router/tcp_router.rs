...
use core::ops::Deref;
use ockam_core::{async_trait, compat::sync::Arc};
use ockam_core::{
    Address, Any, Decodable, LocalMessage, Mailbox, Mailboxes, Result, Routed, Worker,
};
use ockam_node::access_control::{LocalDestinationOnly, LocalOriginOnly};
...
impl TcpRouter {
    ...
    pub async fn register(ctx: &Context) -> Result<TcpRouterHandle> {
        ...
        let main_mailbox = Mailbox::new(
            main_addr.clone(),
            Arc::new(LocalOriginOnly), // TODO: @ac Allow only from the main Router
            Arc::new(LocalDestinationOnly), // TODO: @ac Sending messages to SendWorkers, allow only local destination for now
        );
