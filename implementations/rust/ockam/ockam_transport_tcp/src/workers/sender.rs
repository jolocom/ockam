...
impl Worker for TcpSendWorker {
    ...
    async fn initialize(&mut self, ctx: &mut Self::Context) -> Result<()> {
        ...
        let mailbox = Mailbox::new(
            self.rx_addr().clone(),
            Arc::new(DenyAll),
            Arc::new(LocalDestinationOnly), // (self.internal_addr.clone())), // Sending to this sender and workers that can receive messages from TCP
        );
